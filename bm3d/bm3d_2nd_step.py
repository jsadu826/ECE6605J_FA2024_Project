...
def bm3d_2nd_step(sigma, img_noisy, img_basic, nWien, kWien, NWien, pWien, tauMatch, useSD, tau_2D):
    ...
    print('2nd: input noisy image size:', img_noisy.shape)
    print('2nd: input coarsely denoised image size:', img_basic.shape)
    print('2nd: row/col indices of top N_H most similar patches to the referred patch, size:', ri_rj_N__ni_nj.shape)
    print('2nd: according to tauMatch how many patches are similar to the referred one, bounded by N_H, size:', threshold_count.shape, '| max count:', threshold_count.max(), '| min count:', threshold_count.min())
    group_len = int(np.sum(threshold_count))
    print('2nd: total number of similar patches in the noisy image:', group_len)
    ...
    print('2nd: all patches in the noisy image, size:', noisy_patches.shape)
    print('2nd: all patches in the coarsely denoised image, size:', basic_patches.shape)
    if tau_2D == 'DCT':
    ...
    print('2nd: all patches in the noisy image, after 2d transform, size:', fre_noisy_patches.shape)
    print('2nd: all patches in the coarsely denoised image, after 2d transform, size:', fre_basic_patches.shape)
    ...
    print('2nd: output denoised image size:', img_denoised.shape)
    return img_denoised

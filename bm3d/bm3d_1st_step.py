...
def bm3d_1st_step(sigma, img_noisy, nHard, kHard, NHard, pHard, lambdaHard3D, tauMatch, useSD, tau_2D):
    ...
    # print('1st: input noisy image size:', img_noisy.shape)
    # print('1st: row/col indices of top N_H most similar patches to the referred patch, size:', ri_rj_N__ni_nj.shape)
    # print('1st: according to tauMatch how many patches are similar to the referred one, bounded by N_H, size:', threshold_count.shape, '| max count:', threshold_count.max(), '| min count:', threshold_count.min())
    group_len = int(np.sum(threshold_count))
    # print('1st: total number of similar patches in the noisy image:', group_len)
    ...
    # print('1st: all patches in the image, size:', all_patches.shape)
    if tau_2D == 'DCT':
        fre_all_patches = dct_2d_forward(all_patches)
    else:  # 'BIOR'
        fre_all_patches = bior_2d_forward(all_patches)
    # print('1st: all patches in the image, after 2d transform, size:', fre_all_patches.shape)
    
    # # save patch and its spectra
    # import matplotlib.pyplot as plt
    # plt.figure(figsize=(6, 6))
    # plt.imshow(all_patches[56][56], cmap='viridis', origin='upper')
    # plt.colorbar(label='Intensity')
    # plt.title("Sample 2D Image with Color Bar")
    # plt.axis('off')
    # output_file = 'tmp/1st_step_sample_image_with_colorbar.png'
    # plt.savefig(output_file, bbox_inches='tight', dpi=300)
    # plt.close()
    # # ------------------------------
    # plt.figure(figsize=(6, 6))
    # plt.imshow(fre_all_patches[56][56], cmap='viridis', origin='upper')
    # plt.colorbar(label='Intensity')
    # plt.title("Sample 2D Spectra with Color Bar")
    # plt.axis('off')  # Turn off axes for cleaner visualization
    # output_file = 'tmp/1st_step_sample_spectra_with_colorbar.png'
    # plt.savefig(output_file, bbox_inches='tight', dpi=300)
    # plt.close()
    ...
    # print('1st: output coarsely denoised image size:', img_basic.shape)
    return img_basic
